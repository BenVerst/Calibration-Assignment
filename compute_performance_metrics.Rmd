...
compute_highflowmetrics(m=sager$model,o=sager$obs, month=sager$month, day=sager$day, year=sager$year, wy=sager$wy, high_flow_months = c(5:7))
...
perf = compute_highflowmetrics(m=sager$model,o=sager$obs, month=sager$month, day=sager$day, year=sager$year, wy=sager$wy, high_flow_months = c(5:7))
...
# many ideas -  maybe 50% of mean daily summer observed high flow
tmp = sager %>% subset(month %in% c(5:7)) 
...
tmp = sager %>% subset(month %in% c(5:7)) %>% group_by(wy, month) %>% summarize(obs=sum(obs))
...
perf
# or weight differently - we know that maximum flows are hard to get to weight those differently
